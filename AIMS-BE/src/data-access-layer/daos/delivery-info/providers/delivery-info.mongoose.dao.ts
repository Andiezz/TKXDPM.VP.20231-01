...
export class DeliveryInfoMongooseDao implements DeliveryInfoDao {
    constructor(private deliveryInfoModel: Document = DeliveryInfoModel.getInstance()) {}
    ...
    public async create(createDeliveryInfoDto: CreateDeliveryInfoDto): Promise<IDeliveryInfo> {
    ...
    public async updateInfo(deliveryInfoId: string, updateDeliveryInfoDto: CreateDeliveryInfoDto): Promise<IDeliveryInfo> {
        const deliveryInfoDoc = await this.deliveryInfoModel.findById(deliveryInfoId)
        if (!deliveryInfoDoc) {
            throw new BadRequestError('deliveryInfo is not existed')
        }
        const deliveryInfoNewDoc = await this.deliveryInfoModel.findByIdAndUpdate(deliveryInfoId, { ...updateDeliveryInfoDto})
    ...
    public async getLatestDeliveryInfoId(filter?: Object): Promise<string | null> {
